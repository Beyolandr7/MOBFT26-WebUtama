...
// 1. Group Types
export const groupTypes = mysqlTable("group_types", {
...
export const groups = mysqlTable("groups", {
  ...
  groupTypeId: bigint("group_type_id", { mode: "number" })
    .notNull()
    .references(() => groupTypes.id, { onDelete: "cascade", onUpdate: "cascade" }),
  name: varchar("name", { length: 191 }).notNull(),
  createdAt: timestamp("created_at", { mode: 'string' }),
  updatedAt: timestamp("updated_at", { mode: 'string' }),
  deletedAt: timestamp("deleted_at", { mode: 'string' }),
...
export const groupparticipant = mysqlTable("_groupparticipant", {
  a: bigint("A", { mode: "number" }).notNull().references(() => groups.id, { onDelete: "cascade", onUpdate: "cascade" }),
  b: bigint("B", { mode: "number" }).notNull().references(() => participants.id, { onDelete: "cascade", onUpdate: "cascade" }),
}, (table) => ({
...
export const groupuser = mysqlTable("_groupuser", {
  a: bigint("A", { mode: "number" }).notNull().references(() => groups.id, { onDelete: "cascade", onUpdate: "cascade" }),
  b: bigint("B", { mode: "number" }).notNull().references(() => users.id, { onDelete: "cascade", onUpdate: "cascade" }),
}, (table) => ({
...
export const rolepermissions = mysqlTable("_rolepermissions", {
  a: bigint("A", { mode: "number" }).notNull().references(() => permissions.id, { onDelete: "cascade", onUpdate: "cascade" }),
  b: bigint("B", { mode: "number" }).notNull().references(() => roles.id, { onDelete: "cascade", onUpdate: "cascade" }),
}, (table) => ({
...
export const userroles = mysqlTable("_userroles", {
  a: bigint("A", { mode: "number" }).notNull().references(() => roles.id, { onDelete: "cascade", onUpdate: "cascade" }),
  b: bigint("B", { mode: "number" }).notNull().references(() => users.id, { onDelete: "cascade", onUpdate: "cascade" }),
}, (table) => ({
...
export const assignmentSubmissions = mysqlTable("assignment_submissions", {
  ...
  assignmentId: bigint("assignment_id", { mode: "number" })
    .notNull()
    .references(() => assignments.id, { onDelete: "cascade", onUpdate: "cascade" }),
...
export const attendanceSessions = mysqlTable("attendance_sessions", {
  id: bigint("id", { mode: "number" }).primaryKey().autoincrement().notNull(),
  groupTypeId: bigint("group_type_id", { mode: "number" }).notNull().references(() => groupTypes.id, { onDelete: "cascade", onUpdate: "cascade" }),
  name: varchar("name", { length: 191 }).notNull(),
...
export const attendances = mysqlTable("attendances", {
  id: bigint("id", { mode: "number" }).primaryKey().autoincrement().notNull(),
  participantId: bigint("participant_id", { mode: "number" }).notNull().references(() => participants.id, { onDelete: "cascade", onUpdate: "cascade" }),
  attendanceSessionId: bigint("attendance_session_id", { mode: "number" }).notNull().references(() => attendanceSessions.id, { onDelete: "cascade", onUpdate: "cascade" }),
  recordedByUserId: bigint("recorded_by_user_id", { mode: "number" }).notNull().references(() => users.id, { onDelete: "cascade", onUpdate: "cascade" }),
  status: mysqlEnum("status", ["present", "excused", "absent"]).notNull().default("present"),
  notes: text("notes"),
  createdAt: timestamp("created_at", { mode: 'string' }),
  updatedAt: timestamp("updated_at", { mode: 'string' }),
}, (table) => ({
  partSessionUnique: uniqueIndex("attendances_participant_id_attendance_session_id_key").on(table.participantId, table.attendanceSessionId),
}));
...
// 16. Password Changes
export const passwordChanges = mysqlTable("password_changes", {
  id: bigint("id", { mode: "number" }).primaryKey().autoincrement().notNull(),
  userId: bigint("user_id", { mode: "number" }).notNull().references(() => users.id, { onDelete: "cascade", onUpdate: "cascade" }),
  changedAt: timestamp("changed_at", { mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
  createdAt: timestamp("created_at", { mode: 'string' }),
  updatedAt: timestamp("updated_at", { mode: 'string' }),
}, (table) => ({
...
export const violations = mysqlTable("violations", {
  id: bigint("id", { mode: "number" }).primaryKey().autoincrement().notNull(),
  participantId: bigint("participant_id", { mode: "number" }).notNull().references(() => participants.id, { onDelete: "cascade", onUpdate: "cascade" }),
  violationTypeId: bigint("violation_type_id", { mode: "number" }).notNull().references(() => violationTypes.id, { onDelete: "cascade", onUpdate: "cascade" }),
  recordedByUserId: bigint("recorded_by_user_id", { mode: "number" }).notNull().references(() => users.id, { onDelete: "cascade", onUpdate: "cascade" }),
  notes: text("notes"),
  ...
  updatedAt: timestamp("updated_at", { mode: 'string' }),
  deletedAt: timestamp("deleted_at", { mode: 'string' }),
  createdBy: bigint("created_by", { mode: "number" }).references(() => users.id, { onDelete: "set null", onUpdate: "cascade" }),
});